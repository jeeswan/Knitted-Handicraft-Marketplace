    ...
    <div className='border-t pt-16'>
      ...
      <div>
       ...
       <div className='w-fit  mx-auto mt-7'>
          ...
          className='px-1 py-2 text-[11px] sm:text-[15px] border mx-auto border-gray-500  disabled:opacity-50'
          >Prev</button>
     
            {pageNumbers.map((page)=>(
              <button key={page} onClick={()=>handlePageClick(page)}
               className={`px-3 py-2 border text-[11px] sm:text-[15px] border-gray-500 ${currentPage===page ? 'bg-[green]':''}`}
               >{page}</button>
            ))}
         
          <button onClick={handleNextPage}
          disabled={currentPage===totalPages}
          className='px-1 py-2 border text-[11px] sm:text-[15px] border-gray-500  disabled:opacity-50'
          >Next</button>
        </div>
    </div>
  )
